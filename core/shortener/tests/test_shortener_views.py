...
class TestShortenerView(TestCase):
    ...
    def test_shortener_urls_anonymous_response(self):
        ...
        url_detail = reverse(
            "shortener:detail", kwargs={"pk": self.shortener.pk}
        )
        url_create = reverse("shortener:create")
        url_delete = reverse(
            "shortener:delete", kwargs={"pk": self.shortener.pk}
        )
    ...
    def test_shortener_list_view(self):
        ...
        self.assertNotEqual(
            str(response.content).find(self.shortener.original_url), -1
        )
    ...
    def test_shortener_detail_url_successful_response(self):
        ...
        self.assertEquals(response.status_code, 200)
        self.assertNotEqual(
            str(response.content).find(self.shortener.short_url), -1
        )
    ...
    def test_shortener_create_view(self):
        ...
        response_invalid = self.client.post(
            url, data=invalid_data, follow=True
        )
        ...
        self.assertNotEqual(
            str(response_valid.content).find(data["original_url"]), -1
        )
        for template in ("shortener/shortener_list.html", "base.html"):
    ...
    def test_shortener_redirect_view(self):
        ...
        url = reverse(
            "shortener:redirect", kwargs={"short_url": shortener.short_url}
        )
